...
class AuthenticationPageController extends GetxController {
  ...
  TextEditingController gmailController = TextEditingController(text: "nayeme19@gmail.com");
  TextEditingController passwordController = TextEditingController(text: "nayeme19");

  Future<void> login(BuildContext context) async {
    try {
      setLoadingState(true);
      String? userData = await FirebaseServices.login(context, mail: gmailController.text.trim(), pass: passwordController.text.trim());
      if (userData != null) {
        setLoadingState(false);
        bool isVerified = await FirebaseServices.checkAccountVerificationState(context);
        if (isVerified) {
  ...
  Future<void> register(BuildContext context) async {
    try {
      setLoadingState(true);
      final result = await FirebaseServices.register(context, username: usernameController.text.trim(), mail: gmailController.text.trim(), pass: passwordController.text.trim());
  ...
  Future<void> resetPassword(BuildContext context) async {
    if (gmailController.text.trim().isNotEmpty && !isResetMailSent) {
      await FirebaseServices.resetPassword(context, mail: gmailController.text.trim());
                ...
                child: Column(
                  children: [
                    MyText(
                      ...
                      style: MyTextStyles.titleBold.copyWith(color: AppColor.primaryText, letterSpacing: 3, height: 2),
                    ),
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              MyText(
                                text: "dev_name".tr,
                                style: MyTextStyles.titleBold.copyWith(color: AppColor.primary, letterSpacing: 3),
                              ),
                              const SizedBox(height: 10),
                              MyText(
                                text: "dev_expert".tr,
                                style: MyTextStyles.body.copyWith(color: AppColor.primaryText, letterSpacing: 1.5, height: 2.2),
                              ),
                              MyText(
                                text: "dev_work".tr,
                                style: MyTextStyles.body.copyWith(color: AppColor.primaryText, letterSpacing: 1.5, height: 2.2),
                              ...
                              MyText(
                                text: "dev_mobile".tr,
                                isSelectable: true,
                                style: MyTextStyles.small.copyWith(color: AppColor.secondaryText, letterSpacing: 1.5, height: 2),
                              ),
                              MyText(
